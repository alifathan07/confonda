...
export const createEncaissement = async (req, res) => {
 ...
 // Utility function to convert French number format string to float
const parseFrenchNumber = (numberString) => {
  if (!numberString || typeof numberString !== 'string') return null;
  // Remove spaces and replace comma with decimal point
  const cleanedNumber = numberString.replace(/\s/g, '').replace(',', '.');
...
export const updateEncaissement = async (req, res) => {
  ...
  const encaissement = await prisma.encaissementRecu.update({
    ...
    data: {
      dateEtablissement: dateEtablissement ? new Date(dateEtablissement) : null,
      type,
      montant: parseFrenchNumber(montant), // Convert French number to float
      nDeFactureRG: nDeFactureRG || "",
      RG: RG || "",
      Ras_TVA: Ras_TVA || "",
      Autres: Autres || "",
      ResteAPayer: ResteAPayer, // Convert if also in French format
      banque: { connect: { id: Number(existBanque.id) } },
      client: { connect: { id: Number(existClient.id) } },
      chantier: chantierId ? { connect: { id: Number(chantierId) } } : undefined,
    },
  });
