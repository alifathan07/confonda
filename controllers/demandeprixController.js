...
export const generateDemandePrixPDF = async (req, res) => {
  ...
  try {
    ...
    const pdfBuffer = await generateDemandePrixPDFBuffer(demandePrix);
...
const generateDemandePrixPDFBuffer = async (demandePrix) => {
...
export const sendDemandePrixEmail = async (req, res) => {
  ...
  try {
    ...
    // Generate PDF buffer using new helper function
    const pdfBuffer = await generateDemandePrixPDFBuffer(demandePrix);
    ...
    if (emailResult.success) {
      ...
      return res.status(500).json({ success: false, error: emailResult.error || "Erreur lors de l'envoi de l'email" });
    }
  } catch (error) {
    console.error('Erreur envoi email demande de prix:', error);
    return res.status(500).json({ success: false, error: 'Erreur serveur' });
  }
};
